...
> **限制**：`cna-news-latest-newslist`、`cna-news-latest-category`、`cna-word-map` 與四個 `render-*` 視覺化工具未實測；`fact-check-publish` 沿用 PR #25 的實測結論未重複呼叫。534 則「相異查核」是以回覆前 100 字去重的估計值，非精確去重。題型分布為關鍵字啟發式分類（優先序取第一命中），僅供量級參考。
...
\* 災害類被分類優先序低估；實際上災害題常同時命中政治／政策而被歸到該類。
...
- ❌ 「涵蓋率會提升」——534 則查核裡只有 2 則是「只引用了弱來源」，覆蓋率不是瓶頸。
